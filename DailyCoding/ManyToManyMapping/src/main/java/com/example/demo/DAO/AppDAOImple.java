...
public class AppDAOImple implements AppDAO{
	...
	public Course findCourseAndReviewByCourseId(int theId) {
		TypedQuery<Course> theQuery = entityManager.createQuery("Select c from Course c "+"left Join fetch c.reviews "+" where c.id=:data",Course.class);
	...
	@Override
	public Student findStudentByIdWithCouses(int theId) {
		TypedQuery<Student> theQuery = entityManager.createQuery("Select s from Student s "+"left Join fetch s.courses "+" where s.id=:data",Student.class);
		theQuery.setParameter("data",theId);
	...
	@Override
	public Course findCourseByIdWithStudents(int theId) {
		TypedQuery<Course> theQuery = entityManager.createQuery("Select c from Course c "+"left Join fetch c.students "+" where c.id=:data",Course.class);
		theQuery.setParameter("data",theId);
